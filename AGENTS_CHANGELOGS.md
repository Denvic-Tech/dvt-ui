...
- `LoadExcel`: чтение колонок переведено на синхронный эндпойнт `POST .../graph/nodes/{node_id}/excel-columns` (новый `api.ts` + `loadExcelColumnsApi`). Кнопка «обновить» у поля «Столбцы» теперь отправляет ТЕКУЩИЕ значения модалки (путь, лист, `header_row`), поэтому смена строки заголовка сразу даёт правильные колонки — раньше через метаданные ноды читались сохранённые значения и колонки не появлялись.
- Свежепрочитанные колонки (`fetchedColumns`) имеют приоритет над метаданными; при ошибке чтения показывается подсказка под полем. Подпись у `header_row` уточнена: «Строка с названиями колонок». Удалена зависимость от `useProjectCache` в этом редакторе.

### 2026-09-10 11:12:18

- Нода `LoadExcel`: поле «Столбцы (usecols)» теперь селект по реальным колонкам файла вместо ручного ввода. Колонки берутся из метаданных ноды (`useNodeMetadata` → `output.columns`), которые backend считает через `infer_metadata` (чтение шапки файла). Добавлена кнопка «обновить» (форсирует пересчёт метаданных через `useProjectCache().clearMetadataCache`) и автозагрузка при открытии, если источник задан. Если колонок ещё нет (файл не выбран / glob) — остаётся текстовый ввод как fallback.
- В редакторе типов столбцов (`ColumnDtypeOverridesEditor`) имя колонки тоже стало селектом по колонкам файла (новый проп `columnOptions`); без списка колонок — прежний текстовый ввод.
- Файлы: `src/node-extensions/extract/load-excel/ui/LoadExcelEditor.tsx`, `src/features/node/column-dtype-overrides/ui/ColumnDtypeOverridesEditor.tsx`. Тест редактора дополнен моками `get-node-metadata`/`project-cache`.
...
- Исправлено повторное открытие `Read Table DB V3`: асинхронная загрузка или временная неактуальность metadata больше не удаляет сохранённые `partition_col`, `partition_grouping`, `npartitions` и `max_rows_per_partition`.
- Проверка сохранённой колонки сегментации перенесена в валидацию: после загрузки достоверной схемы отсутствующая колонка показывается как ошибка вместо скрытого изменения конфигурации.
- Колонка сегментации теперь выбирается из полного набора колонок таблицы независимо от output-selection; несовместимая grouping-конфигурация очищается только после явного выбора пользователем другой колонки.
- Добавлены regression-тесты для lazy table metadata lifecycle, stale connection metadata и валидации отсутствующей сохранённой колонки сегментации.
